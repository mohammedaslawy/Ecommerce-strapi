...
const Cart = () => {
  ...
  const deleteCartItemFromList = (id) => {
    CartApis.deleteCartItem(id).then((res) => {
      if(res) setCart((oldCart) => oldCart.filter(item => item.
        id !== res?.data?.data?.id))
    }).catch(error => {
      ...
      <div className="mt-8">
        <ul className="space-y-4">
          {cart.map((item) => (
            ...
            <img
                src={item?.product?.attributes?.banner?.
                data?.attributes?.url}
                alt=""
        ...
        <h2 className='bg-gray-300 text-[14px] mt-5 text-primary'>Note: All Products will be Sent Via Email</h2>
      </div>
    </div>
  </div>
</section>
